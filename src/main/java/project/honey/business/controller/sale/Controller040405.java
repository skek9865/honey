...
import project.honey.business.service.sale.Service040405;
...
@RequestMapping("/040405")
public class Controller040405 {
    
    private final Service040405 service040405;
    ...
    public String findAll(@ModelAttribute("menuId") MenuIdDto menuIdDto,
        ...
        Page<Dto040405> resultList = service040405.findAllByDsl(search, pageable);

        int amt = 0, vat = 0, tot = 0;
        for(Dto040405 dto : resultList) {
            amt += dto.getAmt();
            vat += dto.getVat();
        }
    ...
    public void excel(@ModelAttribute("search") Search040404 search,
        ...
        List<List<String>> excelData = service040405.findAllByExcel(search);
        String fileName = "매입청구서현황(040405).xlsx";

        excelMaker.makeExcel("매입청구서현황 (040405)", titles, excelData, excelType, fileName, response);
    }
}
