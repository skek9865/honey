...
import project.honey.stock.service.Service060103;
...
@RequestMapping("/060103")
public class Controller060103 {

    private final Service040103 service040103;
    private final Service040109 service040109;
    private final Service060103 service060103;
    ...
    public String findAll(@ModelAttribute("menuId") MenuIdDto menuIdDto,
        ...
        Page<Tb603Dto> result = service060103.findAll(search, pageable);
        List<Tb603Dto> content = result.getContent();
        // 푸터
        int qty = 0;
        for (Tb603Dto dto : content) {
    ...
    @PostMapping("/insert")
    public String insert(@ModelAttribute Tb603Form dto, Model model, HttpServletRequest request) {
        log.info("dto : {}", dto);
        model.addAttribute("msg", service060103.insert(dto) != null ? "정상적으로 저장 되었습니다." : "문제가 발생 하였습니다.");
        model.addAttribute("url", request.getHeader("referer"));
        return "redirect";
    }

    @PostMapping("/update")
    public String update(@ModelAttribute Tb603Form dto, Model model, HttpServletRequest request) {
        log.info("dto : {}", dto);
        model.addAttribute("msg", service060103.update(dto) != null ? "정상적으로 저장 되었습니다." : "문제가 발생 하였습니다.");
        model.addAttribute("url", request.getHeader("referer"));
        return "redirect";
    }
    ...
    public String delete(@PathVariable Integer seq, HttpServletRequest request, Model model) {
        model.addAttribute("msg", service060103.delete(seq) != null ? "정상적으로 삭제 되었습니다." : "문제가 발생 하였습니다.");
        model.addAttribute("url", request.getHeader("referer"));
        return "redirect";
    }
    ...
    public String input(Model model,String action, Integer seq){
        ...
        List<String> titles = GlobalMethod.makeTitle(
                "삭제","코드", "품목","수량","대체코드", "대체품목", "정상수량", "불량유형", "적요", "컬러"
        );

        model.addAttribute("statuses", service990301.findByFstId("12"));
        model.addAttribute("prcsmTds", service990301.findByFstId("16"));
...
//       model.addAttribute("emps", service020101.findAllBySelect());
        ...
        Tb603Form form = service060103.findById(seq);
        List<Tb603_1Dto> dtoList = form.getTb603_1Dtos();
        int qty = 0;
        int rQty = 0;
        for (Tb603_1Dto dto : dtoList) {
        ...
        return "stock/060103_input";
    }

    @GetMapping("/excel")
    public void excel(Search060103 search, HttpServletResponse response, HttpServletRequest request) throws IOException {
        log.info("불량처리관리 excel");
        log.info("url = {}", request.getHeader("referer"));

        List<String> titles = GlobalMethod.makeTitle(
                "순번", "처리순번", "창고", "품목", "수량","처리방법", "상태"
        );
        List<String> excelType = GlobalMethod.makeExcelType(
                "String", "String", "String", "int", "String", "String"
        );

        List<List<String>> excelData = service060103.findAllByExcel(search);
        String fileName = "불량처리관리(060103).xls";

        excelMaker.makeExcel("불량처리관리 (060103)", titles, excelData, excelType, fileName, response);
    }
}
